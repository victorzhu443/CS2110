...
public class ScissorsWeights {
    ...
    public static Weigher<ImageEdge> makeWeigher(String weightName, ImageGraph graph) {
        return switch (weightName) {
            ...
            case "CrossGradColor" -> new CrossGradColorWeight(graph); // TODO A6.4b: Register the new weigher
    ...
    static class CrossGradColorWeight implements Weigher<ImageEdge> {
        ...
        private final Raster colorImage;
        ...
        public int weight(ImageEdge edge) {
            ...
            // Compute crossGrad for each band and sum them
            int totalGrad = 0;
            for (int b = 0; b < colorImage.getNumBands(); b++) {
                totalGrad += crossGrad(colorImage, x, y, b, edge.dir());
            }

            // Subtract the summed gradient from the maximum to create a weight
            return eGradMax - totalGrad;
        }
    }
}
