...
public class PathfindingSnapshot {
    ...
    public List<Integer> pathTo(int dstId) {
        if (!discovered(dstId)) {
            throw new IllegalArgumentException("No path discovered to the destination ID: " + dstId);
        }

        List<Integer> path = new ArrayList<>();
        for (int current = dstId; current != -1; current = predecessors[current]) {
            path.add(0, current); // Add at the beginning to reverse the path
        }
